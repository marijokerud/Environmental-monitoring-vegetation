...
#setwd("C://Users//mari.jokerud//NINA//22202020 - Snøhvit vegetasjon - Documents//5. Data og statistikk//Kjemi//Rkode")

#################### TUNGMETALLER ####################
dat1 <- read.table("jord.txt",header=T,dec=",",row.names=NULL)
dat2 <- read_excel("NILU_kjemi2021.xlsx", sheet = "TungR")
summary(dat1)

dat <- dat1 %>% 
  bind_rows(dat2) %>% 
  select(Provetype, Felt, Lok, Aar, Pb, Ni, Zn, Hg) %>% 
  filter(Lok == "Forsol")
